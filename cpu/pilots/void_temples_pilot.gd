...
class_name ArcanaVoidTemplesPilot

# Port of sim/pilots/void_temples.py (Void is unmodeled in the sim; this
# pilot focuses on temple sequencing and cheap birds.)
...
func mulligan(_host: Node, snap: Dictionary) -> bool:
	...
	if not vals.has(1):
		return true
	if rituals.size() >= 4 and not vals.has(4) and not vals.has(3):
		return true
	return false
